...
float computePID(PID& pid, float setpoint, float input) {
...
void updateServoFromMPU() {
  ...
  float pitchOutput = computePID(pitchPID, 0, gy);  // Target angle 0 (level)
  float rollOutput = computePID(rollPID, 0, gx);    // Target angle 0 (level)
