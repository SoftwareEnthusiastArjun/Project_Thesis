...
void calibrate(){
  ...
  for (x = 0; x < num; x++){
    ...
    error = i2c_read(MPU6050_I2C_ADDRESS, 0x43, i2cData, 6);//(The I²C address of the MPU-6050 device(addr),The register address to start reading from(start)
                                                            // , Where the read data will be stored(buffer), Number of bytes to read (2 bytes per axis × 3 axes)) 
    if(error!=0)
    return;
