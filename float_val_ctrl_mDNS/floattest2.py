...
import threading
import time
"""Now I dont have to know IP address to change the float val, mDNS will take care of it.
This is a simple GUI application to control a float value on an ESP32 device using a slider.
 The application connects to the ESP32 over TCP/IP and allows the user to set and save a float value to EEPROM.
   The GUI updates in real-time based on the current value from the ESP32."""
class FloatValueGUI:
    def __init__(self, root):
        ...
        self.esp_ip = "esp32.local"  # Using mDNS instead of static IP
        self.esp_port = 12345
    ...
    def connect_to_esp32(self):
        try:
            # Resolve esp32.local to IP address
            resolved_ip = socket.gethostbyname(self.esp_ip)
            print(f"Resolved IP: {resolved_ip}")
            
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(2)
            sock.connect((resolved_ip, self.esp_port))
